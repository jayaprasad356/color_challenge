...
class ContestChallenge extends StatefulWidget {



  const ContestChallenge({Key? key}) : super(key: key);

  @override
  State<ContestChallenge> createState() => ContestChallengeState();
}

class ContestChallengeState extends State<ContestChallenge> {
  ...
  Future<void> taskList() async {
    ...
    setState(() {
      ...
      desc =
      "See Number then type in box and finish in best time.";
    });
  }
  ...
  Future<void> declareResult() async {
    ...
    Map<String, dynamic> bodyObject = {
      Constant.USER_ID: prefs.getString(Constant.ID),
      Constant.TIME: _seconds.toString(),
    };
    String jsonString = await apiCall(url, bodyObject);
    final jsonResponse = jsonDecode(jsonString);
    final message = jsonResponse['message'];
